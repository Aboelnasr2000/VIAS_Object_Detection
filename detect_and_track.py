...
def draw_boxes(img, bbox, identities=None, categories=None, names=None, save_with_object_id=False, path=None,
    ...
    for i, box in enumerate(bbox):
        ...
        (w, h), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 10)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for x1, y1, x2, y2, conf, detclass in det.cpu().detach().numpy():
                    ...
                    ImageWQ = ImageW / 4
                    Center = (x1 + x2) / 2
                    if ImageWQ < Center < ImageW - ImageWQ:
                        # Calculate the distance of the person
                        person_height = 1.7
                        person_width = 0.7
                        xyxy_width = x2 - x1
                        xyxy_height = y2 - y1
                ...
                for track in tracks:
                    ...
                    if colored_trk:
                        [cv2.line(im0, (int(track.centroidarr[i][0]),
                                  ...
                                  rand_color_list[track.id], thickness=10)
